...
function compteTemps() {
	...
	setInterval(function() {
		context4.clearRect(largeurCanvas-(2*tailleTuile)-20, tailleTuile*8, tailleTuile*2.5, tailleTuile*1.5);
...
function gereEnergie() {
	...
	setInterval(function() {
		context4.clearRect(largeurCanvas-(2*tailleTuile)-20, tailleTuile*10, tailleTuile*2.5, tailleTuile*2);
		ajouteTexte("Energie : " + pointsEnergie, 12, largeurCanvas-(2*tailleTuile)-20 , tailleTuile*10 + 20);
...
function gagne() {
	if (nbPoints == 16 && pointsEnergie >= 0) { // si on a le bon nombre de points et que l'énergie est positive
		...
		ajouteTexte("Gagné !", 50, tailleTuile*7 , tailleTuile*5); // on écrit qu'on a gagné
...
function perd() {
		if (nbPoints < 16 || pointsEnergie <= 0) {
			ajouteTexte("Perdu !", 50, tailleTuile*7 , tailleTuile*5);
