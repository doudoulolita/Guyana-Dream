...
function ecranFinJeu() {
	...
	context[1].clearRect(ennemi.posX2, ennemi.posY2-(tailleTuile*1.5), texteEnnemi.length*12, tailleTuile);	// efface le texte de dialogue avec l(ennemiu
	...
	ajouteTexte(texteResultat, 50, tailleTuile*8 , tailleTuile*5); // on écrit qu'on a gagné

	textePnj = textePnjResultat; // change le texte du PNJ dans sa bulle

	bulleTexte(textePnj, pnj.posX2, pnj.posY2); // affiche la bulle du PNJ

	texteEnnemi = texteEnnemiFin;
	bulleTexte(texteEnnemi, ennemi.posX2, ennemi.posY2); // affiche la bulle du PNJ
}
