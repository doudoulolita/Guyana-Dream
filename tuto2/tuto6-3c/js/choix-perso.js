...
function dessineJoueurs() {
	ajouteTexte('Cliquez sur un perso à droite', 14, 20, 20); // texte en haut du canvas avec l'instruction
	for (i=joueurs.length-1; i>=0; i--) { // parcourt le tableau des joueurs
		dessineJoueur(joueurs[i], context[4], tailleTuile*(cartes[0][0].length+i), tailleTuile*7.5, joueurs[i].largeur, joueurs[i].hauteur, Math.floor(joueurs[i].pose), dirJoueur);
	}
...
function clicJoueur() {
	for (i=0; i<joueurs.length; i++) { // parcourt le tableau des joueurs
		if(xSourisCanvas>tailleTuile*(cartes[0][0].length+i) && xSourisCanvas<tailleTuile*(cartes[0][0].length+i)+largeurJoueurMax && ySourisCanvas>tailleTuile*6.5 && ySourisCanvas<tailleTuile*7.5+hauteurJoueurMax) { // vérifie à quel en droit on a cliqué par rapport à l'image de choix du joueur
			...
			animePerso(joueur); // lance l'animation du joueur (fichier animation.js)
