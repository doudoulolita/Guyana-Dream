...
function bloqueBords() {
	...
	if (joueur1.posX < 0) { // si la position horizontale du joueur est inférieure à 0 (à gauche)
		joueur1.posX = 1; // on fixe sa position
		numCarte=1; // on passe à la 1ère carte
		changeCarte(1); // on change de carte
	}
	if (joueur1.posY < 0) { // si la position verticale du joueur est inférieure à 0 (en haut)
		joueur1.posY = 1; // on fixe sa position
	}
	if (joueur1.posX > carte1[0].length*tailleTuile - joueur1.largeur) { // si le joueur dépasse de la carte à droite
		joueur1.posX = carte1[0].length*tailleTuile - joueur1.largeur; // quand les bords sont atteints à l'horizontale, on fixe sa position
		numCarte = 2; // on passe à la deuxième carte
		changeCarte(1); // on change de carte
	}
	if (joueur1.posY > carte1.length*tailleTuile - joueur1.hauteur) { // si le joueur est en bas de la carte
		joueur1.posY = carte1.length*tailleTuile - joueur1.hauteur; // quand les bords sont atteints à la verticale, on fixe sa position
	}
}
...
function bloque(posXTuile, posYTuile) { 
	...
	if (joueur1.posX + joueur1.largeur > posXTuile && joueur1.posX - tailleTuile < posXTuile && joueur1.posY + joueur1.hauteur > posYTuile && joueur1.posY - tailleTuile < posYTuile) { // si les coordonnées du joueur sont situés au bord de la tuile interdite

		switch(dir) {
			case 1:
				joueur1.posX -= 1; // on ajoute sens à x pour qu'il se décale horizontalement et sorte de la tuile 
				break;
			case 2:
				joueur1.posX += 1; // on ajoute sens à x pour qu'il se décale horizontalement et sorte de la tuile 
				break;
	 		...
	 		case 0: 
				joueur1.posY -= 1; // on enlève 1 à y pour qu'il se décale verticalement et sorte de la tuile 
				break;
			case 3:
				joueur1.posY += 1; // on ajoute sens à y pour qu'il se décale verticalementement et sorte de la tuile 
				break;
		}
	}
}
	...
	for (j=ligne; j<cartes[numCarte-1].length; j++) {//cartes[numCarte-1] représente toute la carte
		for(i=colonne; i<cartes[numCarte-1][j].length; i++) {//cartes[numCarte-1][j] représente maintenant une ligne de la carte	
			for (k=numObjet; k<objets.length; k++) {
				if (cartes[numCarte-1][j][i] == objets[k][0] && joueur1.posX > (tailleTuile*i)-joueur1.largeur && joueur1.posX < (tailleTuile*i)+tailleTuile && joueur1.posY > (tailleTuile*j)-joueur1.hauteur && joueur1.posY < (tailleTuile*j)+tailleTuile) {  //si la carte présente un n° de tuile compris dans le tableau d'objet et si ses coordonnées du joueur sont comprises dans les limites de cette tuile
...
function bulleQuete(i, j) {
	...
	if (cartes[numCarte-1][j][i] == objetQuete && pnj.collision!=0) {  //si la carte correspond à la quête du Pnj
		...
		joueur1.vx *= 1.05;	 // la vitesse augmente un peu quand on récupère cet objet
	}
}
