...
function recupCanvas() {
	...
	for (i=numCanvas; i<canvas.length; i++){ // on parcourt le tableau des canvas
	...
	for (i=numCanvas; i<canvas.length; i++) {//pourquoi faire une boucle ? car plus tard, nous rajouterons encore des canvas par dessus! on n'aura donc pas besoin de réadapter le code
...
function initialisePoints() {
	...
	vx = joueurs[0].vx; // vitesse de déplacement du personnage
	cartes = [carte1];
	ajouteCarte();
}
