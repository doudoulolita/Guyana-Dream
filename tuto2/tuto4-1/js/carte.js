...
function bloqueBords() {
	...
	if (joueur.posX > canvas.width - joueur.largeur) { // si le joueur dépasse de la carte à droite
		joueur.posX = canvas.width - joueur.largeur; // quand les bords sont atteints à l'horizontale, on fixe sa position
	}
	if (joueur.posY > canvas.height - joueur.hauteur) { // si le joueur est en bas de la carte
		joueur.posY = canvas.height - joueur.hauteur; // quand les bords sont atteints à la verticale, on fixe sa position
	}
}
